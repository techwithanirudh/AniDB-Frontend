...
fetch('https://anidd-backend.techwithanirudh.repl.co/api/v1/create/' + database, {
	method: 'POST',
});
fetch('https://AniDB-Backend.techwithanirudh.repl.co/api/v1/get/' + database + '/' + name).then(
...
function main(data) {
	...
	function exportTableToDictionary(bodyEls) {
		var table = {};
	...
	saveButton.onclick = function () {
		...
		(async () => {
			const rawResponse = await fetch('https://anidb-backend.techwithanirudh.repl.co/api/v1/create/' + database + '/' + name, {
				method: 'POST',
				headers: {
